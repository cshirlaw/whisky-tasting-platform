...
function isBlendCategory(category: string) {
  const c = String(category || "").toLowerCase();
  return c === "blended scotch whisky".toLowerCase() || c === "blended-malt-scotch";
}

export default async function BottlesIndexPage() {
  const items = await loadBottleSummaries();

  const blends = items.filter((x) => isBlendCategory(x?.bottle?.category || ""));
  const nonBlendWithTastings = items.filter((x) => !isBlendCategory(x?.bottle?.category || "") && x.tastingCount > 0);
    ...
    <main className="mx-auto max-w-5xl px-4 py-10">
      ...
      {items.length === 0 ? (
        ...
        <>
          ...
          {nonBlendWithTastings.length > 0 ? (
            <p className="mt-8 text-sm text-neutral-600">
              Some non-blend whiskies have tastings recorded and are kept in a separate expert area.
            </p>
          ) : null}
        </>
      )}
